...
@description('v2.10.0 Infrastructure tab — CSV of Azure subscription IDs whose VMs the admin surface should manage. Passed to the backend as INFRA_SUBSCRIPTIONS (pydantic-settings maps `infra_subscriptions` field → uppercase env var, no RTD_ prefix — v2.10.3 fix). Empty → tab is inert (backend flips to mock mode when env=local).')
...
var appEnv = [
  ...
  { name: 'INFRA_SUBSCRIPTIONS', value: infraSubscriptions }
]
