...
Per-Project streams give us natural isolation:
- inbound  ``runs:{project_id}:in``      — run.start / run.resume commands
- outbound ``runs:{project_id}:events``  — lifecycle events for the SSE feed
...
def inbound_stream(project_id: uuid.UUID | str) -> str:
    return f"{_INBOUND_PREFIX}{project_id}{_INBOUND_SUFFIX}"


def outbound_stream(project_id: uuid.UUID | str) -> str:
    return f"{_INBOUND_PREFIX}{project_id}{_OUTBOUND_SUFFIX}"
