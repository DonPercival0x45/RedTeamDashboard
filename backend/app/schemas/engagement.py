...
class EngagementRead(BaseModel):
    ...
    # glance. ``scope_count`` counts in-scope items, ``exclusion_count``
    # counts !-marked items. Populated by ``list_engagements``,
    # ``get_engagement``, and ``update_engagement`` (cheap aggregate,
    # default 0 so existing test fixtures still construct cleanly).
    scope_count: int = 0
