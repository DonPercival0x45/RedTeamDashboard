...
def test_delete_playbook_refuses_when_runs_exist(db: Session, custom_playbook: Playbook) -> None:
...
def test_add_step_auto_sort_order_places_after_max(db: Session, custom_playbook: Playbook) -> None:
...
def test_add_step_respects_explicit_sort_order(db: Session, custom_playbook: Playbook) -> None:
    s = add_step(
        db,
        playbook=custom_playbook,
        tool_slug="whois",
...
def test_post_playbooks_creates_playbook(db: Session, client: TestClient, user: User) -> None:
...
def test_post_playbooks_slug_conflict_409(db: Session, client: TestClient, user: User) -> None:
    slug = f"dup-{uuid.uuid4().hex[:6]}"
    client.post(
        "/playbooks",
        headers=_h(user),
...
def test_post_playbooks_guest_blocked(db: Session, client: TestClient, guest_user: User) -> None:
    resp = client.post(
        "/playbooks",
        headers=_h(guest_user),
        json={"slug": "x", "name": "x", "applies_to_asset_class": "domain"},
    )
    assert resp.status_code == 403


def test_patch_playbook_requires_new_version(db: Session, client: TestClient, user: User) -> None:
    ...
    assert "publish a new version" in resp.json()["detail"]


def test_patch_playbook_unknown_404(db: Session, client: TestClient, user: User) -> None:
    resp = client.patch(
        "/playbooks/never",
        headers=_h(user),
        json={"name": "x"},
...
def test_delete_playbook_removes_it(db: Session, client: TestClient, user: User) -> None:
...
def test_delete_playbook_with_runs_409(
    ...
    assert "versions are immutable" in step.json()["detail"]


def test_post_step_endpoint_requires_new_version(
...
def test_legacy_step_editor_is_immutable(client: TestClient, user: User) -> None:
    ...
    assert "versions are immutable" in unknown.json()["detail"]
...
def test_patch_step_endpoint_edits(db: Session, client: TestClient, user: User) -> None:
    ...
    resp = client.patch(
        f"/playbooks/{slug}/steps/{step_id}",
        headers=_h(user),
...
def test_delete_step_endpoint_removes(db: Session, client: TestClient, user: User) -> None:
...
def test_analyst_authored_active_playbook_hits_the_gate(
    ...
    payload = {"playbook_slug": slug, "scope_subset": ["foo.com"]}
    plan = client.post(
        f"/engagements/{eng_slug}/playbook-runs/plan",
        headers=_h(user),
        json=payload,
    )
    assert plan.status_code == 200, plan.text
    resp = client.post(
        f"/engagements/{eng_slug}/playbook-runs",
        headers=_h(user),
