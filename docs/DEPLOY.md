...
**Maltego import (Phase 10):** Maltego is a "heavy tool" per charter §16 RESOLVED — analysts run it on their own infra and `POST /engagements/{slug}/entities/import/maltego` the `.mtgx` graph export (multipart `file=`). The endpoint parses the zip-of-GraphML server-side via `defusedxml` (XXE/billion-laughs safety; 10MB cap on the GraphML payload to defang zip bombs) and UPSERTs each `MaltegoEntity` into the new `entities` table on conflict key `(engagement_id, type, value)` — re-imports merge JSONB properties (`existing || incoming`) rather than duplicating rows. Imported entities land in a **separate store** from the existing derived-on-the-fly entities (the view at `app/services/entities.py` stays unchanged). Maltego types are normalized to our vocabulary where they overlap (`maltego.Domain → domain`, `maltego.EmailAddress → email`, `maltego.IPv4Address → ip`, etc.); unknown types pass through verbatim so analysts can filter on Maltego-specific types in the UI. The Entities tab now shows two sections: **Imported** (stored, with source attribution) above **Derived from findings** (the existing view). `GET /engagements/{slug}/entities/stored` lists the stored layer with optional `?type=` + `?q=` filters.

**DarkWeb import (Phase 10, Dehashed first):** breach / leak / credential-dump data is import-first like Maltego — analysts run searches on their own infra (Dehashed account, etc.) and upload the export. `POST /engagements/{slug}/entities/import/darkweb?source=dehashed` accepts JSON (the standard API response shape `{entries: [...]}` or a bare array) and CSV (`csv.DictReader` over UTF-8, BOM-tolerant). The framing is deliberately generalized — `app/services/darkweb_import.py` is the home for future sources (HIBP, IntelX, internal corpora); each ships as a new parser hooked into the same endpoint via the `?source=` query. One Entity per breach record (not per populated field) with `type="breach_record"`; the composite value `<email-or-username>@<database_name>` preserves per-breach distinction across multiple leaks of the same identity while UPSERTing cleanly on re-imports. The full record (email, password, hashed_password, name, phone, etc.) lives in `properties` JSONB. Frontend: the Entities tab "Imported" section's file picker auto-routes by extension (`.mtgx` → Maltego, `.json`/`.csv` → DarkWeb); the post-import counts panel adapts per source.
