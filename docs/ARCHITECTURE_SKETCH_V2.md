...
# Project X-Ray — Combined Architecture Sketch (v2)
...
2. **Engagement = tabbed workspace** per target: OSINT Recon · Vuln Scan ·
   Validation · Phishing · Results · Costs, with a Strategic command-center view.
3. **Actionable findings** — every finding is a launch point: click it to see
   the tasks needed to validate it.
4. **Analyst chooses who scans** — agent-run or analyst-run, per scan.
5. **Agents scan; analysts validate.** Hard invariant (see §7).
   ...
   Analyst (Entra SSO) ──►│  DASHBOARD (Next.js SWA, all-black)     │
        ...
        └───────┬───────┘    └───────┬──────────┘
                └──────── results ────┴──► PENDING_VALIDATION
                          ...
                          ┌───────────────────┼───────────────────┐
                          ▼                    ▼                   ▼
                   validation-task list  Results tab → PDF     Cost engine
...
- Generates the **validation-task list** behind each actionable finding (§6).
...
- **Performs all validation**, manually, and **uploads** the results/artifacts.
...
FINDING (validated)
  ...
  └─ ▼ click ─────────────────────────────────────────────┐
       VALIDATION-TASK LIST (generated by Strategic)        │
       • Task: "Test SQLi on /login"   [analyst-only]      │
       • Task: "Enumerate DB users"    [agent-eligible]    │
       • Task: "Spin Kali box to validate" → ephemeral req │
...
## 7. Hard invariant: agents scan, analysts validate

```
            PASSIVE OSINT   ACTIVE ENUM   VALIDATION
Tactical /
worker agent      ✅              ✅ (gated)        ❌  NEVER
Analyst           ✅              ✅                ✅  (then uploads)
```

- No agent — Tactical, worker, or ephemeral — may run a validation task.
  Enforced in the tool registry (tools tagged `validation` are not dispatchable by
  any agent path) **and** at dispatch (Tactical refuses `validation`-class tasks).
- Validation results enter the system only via analyst **upload**, then go
...
6. SUGGEST       Strategic analyzes findings → priced next-task suggestions
                 + per-finding validation-task lists; flags ephemeral needs
7. SELECT        analyst cherry-picks: delegate enum to Tactical, or take
                 validation tasks themselves, or approve an ephemeral box
8. VALIDATE      analyst-only; results uploaded → back to step 4
...
- **Findings** render as actionable cards (click → validation-task list, §6).
...
| Validation | Analyst-only, uploaded | Legal/operational control |
| Scan ownership | Analyst chooses agent vs self, per scan | Flexibility (the "Hybrid" choice) |
| Findings | Actionable; spawn validation-task lists | Drive the next move |
...
| Attack-box | Kali VM, analyst RDP/SSH | Analyst validates from it; agent never does |
